...
export function OrdersListClient({ orders }: { orders: Order[] }) {
  ...
  const filtered = useMemo(
    () => (selected ? orders.filter((o) => o.restaurant.cuisine === selected) : orders),
    [orders, selected],
  );
  ...
  return (
    <>
      <div className="-mx-5 mt-4 flex gap-2 overflow-x-auto px-5 pb-1 scrollbar-hide">
        <Chip active={selected === null} onClick={() => { setSelected(null); setVisible(PAGE_SIZE); }}>
        ...
        {cuisines.map((c) => (
          <button
            key={c}
            type="button"
            onClick={() => { setSelected(c); setVisible(PAGE_SIZE); }}
            className={
          ...
          >
            <CuisineBadge cuisine={c} className={selected === c ? "!bg-white/20 !text-white" : ""} />
      ...
      <div className="mt-5 space-y-3">
        {shown.length === 0 ? (
          <EmptyState icon="🍽️" title="No orders yet" body="Try changing the cuisine filter above." />
        ) : (
  ...
  return (
    <button
      ...
      className={
        ...
        (active ? "bg-accent text-white shadow-card" : "bg-surface text-text-primary shadow-card")
      }
    >
      {children}
    </button>
